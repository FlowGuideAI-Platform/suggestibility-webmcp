...
- Once escalated to someone with auth platform context (09:06), root-causing to the key rotation took only 14 minutes (09:06–09:20), which reflects good dashboard tooling for correlating deploys/jobs against error spikes.
- The rolling restart, while slow (95 minutes), executed without further incidents or secondary outages, and the team maintained a clear, accurate incident log throughout, which made this postmortem substantially easier to write.
- No data integrity or confidentiality impact. The failure mode, while severe, was a clean "closed" failure (nobody got in who shouldn't have) rather than an "open" one.
- Support's ticket-volume-triggered escalation, while slower than ideal, did work as a backstop when the automated alerting gap (factor #3) meant nothing else would have caught this.

## 7a. What Went Poorly

- **40 minutes of blind diagnosis time before the right expertise was in the room.** The on-call SRE is a strong generalist but had never worked on the JWKS/key-rotation subsystem and, by their own account in the incident retro, spent a meaningful fraction of the 08:36–09:06 window reading through auth service source code to understand the token validation path from scratch, in production, under a customer-wide outage. This is not a knock on the individual; it's a direct consequence of contributing factor #1 (no dedicated on-call).
- **The team discovered the rollback path was gone at 09:58, an hour into the incident, rather than knowing it up front.** Nothing in the rotation job's documentation stated that revocation was immediate and irreversible; the team's working assumption going into the mitigation phase was that a rollback was available, and real time was spent pursuing that path (09:44–09:58) before it was ruled out. A design doc or runbook that stated plainly "this rotation cannot be undone once it completes" would have redirected effort toward the forward-fix roughly 15 minutes sooner.
- **Enumerating all 34 services that independently validate tokens was itself a scramble.** There is no single authoritative list of "services that consume the JWKS endpoint and cache it client-side." The list used during the incident was assembled live by asking in the incident channel who owned what, cross-referenced against a service catalog that turned out to be about 85% accurate — two services were missed on the first restart pass and had to be caught in a second sweep at 11:15, adding roughly 25 minutes to full recovery.
- **The status page lagged real customer awareness by over an hour.** Even accounting for the detection delay explaining most of this, a full hour where customers and our own Marketing team were more informed via public social posts than via our own official channel is a real gap in the incident communication process, not just a downstream symptom.

## 8. Action Items

| ID | Action | Owner | Priority | Status |
|---|---|---|---|---|
| AI-1 | Correct JWKS CDN cache TTL to the originally specified 5 minutes | Auth Platform | P0 | Done (shipped day of incident) |
| AI-2 | Add mandatory CDN + client-SDK cache purge as an explicit, automated step in the key rotation job itself, not a manual runbook step | Auth Platform | P0 | In progress |
| AI-3 | Add synthetic end-to-end login/token-validation probe to health checks and paging alerts, independent of 5xx-based SLOs | SRE | P0 | In progress |
| AI-4 | Introduce a key-overlap grace period (both old and new key valid for N minutes post-rotation) to the rotation design | Auth Platform | P1 | Proposed, design review scheduled |
| AI-5 | Update rotation runbook to match current tooling; add quarterly runbook accuracy review to on-call handoff checklist | Auth Platform | P2 | Not started |
| AI-6 | Evaluate a dedicated (even if lightweight, e.g., business-hours-only) on-call path for auth platform escalations | Eng Leadership | P2 | Under discussion |
| AI-7 | Audit all other cache-sensitive endpoints swept up in the 14-month-old CDN TTL cost-optimization pass for similar mismatches | SRE | P1 | Not started |
| AI-8 | Formalize incident communication SLA (status page update within N minutes of SEV-1 declaration, independent of root-cause confirmation) | Support + SRE | P2 | Proposed |

**Note on prioritization disagreement:** AI-4 (key overlap grace period) is filed as P1, one tier below the P0 items, reflecting the auth platform lead's view (Section 4) that the TTL and detection fixes (AI-1, AI-2, AI-3) address the actual failure mode and the overlap window is a defense-in-depth improvement rather than the core fix. The security lead's recorded position is that AI-4 should be P0 and should have been in place independent of this incident, on the general principle that key rotation without an overlap window is a known-bad pattern regardless of whether a specific trigger has caused a specific outage yet. This is unresolved as of publishing; it will be revisited at the design review in AI-4.

## 9. On Blamelessness

This postmortem intentionally does not name the individual who set the 14-month-old CDN TTL change, the individual who built the immediate-revocation behavior into the rotation job, or the individual first-responder. All of these were reasonable decisions or actions given the information available to each person at the time they were made. The TTL change was part of a legitimate, reviewed cost-optimization effort; nobody flagged the JWKS endpoint as an exception at the time because its cache-sensitivity wasn't documented anywhere a reviewer would have seen it. The immediate-revocation design was a considered security trade-off, not an oversight. We are treating both as systemic gaps — missing documentation linking cache config to sensitive endpoints, and a security-vs-availability trade-off that wasn't revisited after initial design — not individual errors.

## 10. Appendix: Metrics Referenced

- `auth.token.validate.failure_rate` (401-classified, by region) — flat baseline until 07:54, step-function increase to ~98% by 08:05, sustained until 11:40–11:52 rolling recovery.
- `auth.jwks.cdn_cache.age_seconds` (added post-incident, did not exist during the incident) — would have shown the stale-cache condition directly; its absence is part of why detection depended on downstream symptoms (401 rate, ticket volume) rather than the cache state itself.
- `support.tickets.category=login_failure` — baseline ~2/hour, peaked at approximately 95/hour during the incident window.

This document will be revisited after AI-4's design review to record the resolution of the P0-vs-P1 disagreement in Section 8, and linked from the rotation runbook once AI-5 is complete.
